...
GOplot <- function(genelist,
  ...
  gene <- bitr(t(genelist), fromType = "SYMBOL", toType = "ENTREZID", OrgDb = "org.Hs.eg.db")
  ego_MF <- enrichGO(OrgDb = "org.Hs.eg.db", gene = gene$ENTREZID, keyType = "ENTREZID", pvalueCutoff = enrich.pvalue, ont = "MF")
  ego_CC <- enrichGO(OrgDb = "org.Hs.eg.db", gene = gene$ENTREZID, keyType = "ENTREZID", pvalueCutoff = enrich.pvalue, ont = "CC")
  ego_BP <- enrichGO(OrgDb = "org.Hs.eg.db", gene = gene$ENTREZID, keyType = "ENTREZID", pvalueCutoff = enrich.pvalue, ont = "BP")
  ...
  go_enrich_df <- data.frame(
    ID = c(go_BP$ID, go_CC$ID, go_MF$ID),
    ...
    type = factor(c(
      rep("biological process", display.number), rep("cellular component", display.number),
      rep("molecular function", display.number)
